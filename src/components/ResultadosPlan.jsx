  ...
  return (
    <div className="h-full bg-surface rounded-xl shadow-sm border border-border p-5 flex flex-col transition-all hover:shadow-md">
      <div className="flex items-center gap-2 mb-6 border-b border-border pb-4">
        ...
        <h2 className="text-xl font-bold text-text">Plan de Ejecución</h2>
      </div>
      ...
      {!results ? (
        ...
        <>
          <div className={`flex items-center gap-3 p-4 rounded-lg mb-6 shadow-sm border ${results.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
            {results.success ? (
              <CheckCircle2 size={24} className="text-green-500" />
            ) : (
              <AlertCircle size={24} className="text-red-500" />
            )}
            <span className={`font-semibold text-lg ${results.success ? 'text-green-700' : 'text-red-700'}`}>
              {results.message}
            </span>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-border bg-white">
                  {results.results.map((r, i) => {
                    const color = getColorForTable(r.fragment);
                    return (
                      <tr key={i} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-3 font-medium flex items-center gap-2">
                          ...
                          <span style={{ color: color.bg }}>{r.fragment}</span>
                        </td>
                      </tr>
                    );
                  })}
